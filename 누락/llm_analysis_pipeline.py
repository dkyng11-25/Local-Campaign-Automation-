...
#
# Excel 입력 및 모든 Excel 결과:
#   누락/output_누락/{YYMMDD}_누락/
#
# 미디어 파일은 media_extractor가 다음 위치에 저장하며,
# 이 모듈은 llm_input의 local_media_paths를 통해 해당 파일을 읽는다.
#   누락/media_누락/{YYMMDD}_누락/
#
# LLM Prompt 및 Response Schema:
#   Local_Campaign_Automation_version4/prompts/
#
# Country–Subsidiary 매핑:
#   Local_Campaign_Automation_version4/config/
#   country_subsidiary_mapping.json
OUTPUT_DIR = BASE_DIR / "output_누락"

# prompts와 config는 누락 서브 폴더가 아니라
# Local_Campaign_Automation_Version4 프로젝트 루트의 공용 폴더를 사용한다.
...
def extract_run_info_from_directory_name(
    ...
    누락 실행은 차수 개념을 사용하지 않지만 기존 반환 타입과
    환경변수 호환성을 유지하기 위해 내부 run_number는 1로 반환한다.
    """
...
def resolve_execution_output_dir(
    ...
    우선순위:
        ...
        3. input_date가 있으면 output_누락/{YYMMDD}_누락
        4. 기본 경로가 필요하지 않고 모든 파일 경로를 직접 지정한 경우 None

    이 함수는 output 날짜 폴더를 새로 생성하지 않는다.
    """
    ...
    output_dir = (
        ...
        or default_missing_output_dir
    )

    if output_dir is None:
        if required:
            raise RuntimeError(
                "실행 output 폴더를 확정할 수 없습니다.\n"
                "누락 서브 파이프라인 단독 실행 시 "
                "--input-date YYMMDD를 지정하거나 실행 후 날짜를 입력하세요.\n"
                "경로를 직접 지정하는 경우:\n"
                "python llm_analysis_pipeline.py "
                '--output-dir "output_누락\\260805_누락"'
            )
    ...
    if output_dir.name.endswith("_누락"):
        print(
            "[INFO] 실행 유형: 누락 데이터 처리"
        )
    else:
        print(
...
def build_default_input_path(
    ...
    """
    같은 누락 output 폴더에서 media_extractor 결과 Excel을 찾는다.

    누락 media_extractor는 원본 파일명을 유지한 채 해당 파일을
    llm_input Excel로 교체하므로 다음 이름을 입력으로 사용한다.
    ...
    return (
        ...
        / (
            ...
            f"{input_date_obj.month}월_v01.xlsx"
        )
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            "누락 실행 output 폴더. "
            "예: output_누락/260805_누락. "
            "미지정 시 입력 날짜를 기준으로 자동 결정합니다."
        ),
    )
    parser.add_argument(
        "--input-excel",
        type=Path,
        help=(
            "media_extractor 결과가 저장된 "
            "{date}_SLCC_SOV_Local Campaign Tracking_{월}월_v01.xlsx 경로"
        ),
    )
    parser.add_argument(
        "--target-excel",
...
def resolve_execution_paths(
    ...
    # --output-dir 또는 파일 경로를 모두 직접 주지 않은 일반 단독 실행에서는
    # output_누락/{YYMMDD}_누락 경로를 만들기 전에 날짜를 먼저 확정한다.
...
def main() -> None:
    ...
    print(
        "Country 매핑 요약: "
        f"total={country_mapping_summary['total']}, "
        f"assigned={country_mapping_summary['assigned']}, "
        f"unassigned={country_mapping_summary['unassigned']}"
    )
    print(
        f"사용자 프롬프트: "

# ===============================================================================================
# 누락 Sprinklr Raw Excel -> Processed Excel
#
# 주요 처리:
# 1. Raw Data_원문 / Raw Data_전략법인 시트를 읽는다.
# 2. 로컬 캠페인 리스트_QHB8 시트를 새로 생성한다.
# 3. 원 게시물 Permalink를 기준으로 comment_extractor를 호출하여
#    소비자 반응 URL을 함께 수집하고 최종 URL 셀 값을 생성한다.
# 4. Influencer 및 Subsidiary 컬럼은 이후 LLM 분석 단계에서 채우도록 비워 둔다.
# 5. Sprinklr Export 단계에서 추가한 Sender Profile 컬럼은 Raw 시트에 그대로 보존된다.
# 6. HTR/DE, Conv.Card 기본값 및 해시태그/Query 처리 규칙은 최신 정상 파이프라인과 동일하게 적용한다.
#
# 누락 전용 경로:
#   누락/output_누락/{YYMMDD}_누락/
#
# 전체 누락 파이프라인에서 LOCAL_CAMPAIGN_OUTPUT_DIR이 전달되면 해당 폴더를 사용하고,
# 단독 실행에서는 --output-dir을 지정하거나 기본 누락 날짜 폴더를 자동으로 찾는다.
# 이 모듈은 새로운 날짜 폴더를 생성하지 않는다.
# ===============================================================================================
...
# comment_extractor.py는 일반적으로 프로젝트 루트에 위치한다.
# 누락 폴더 안에 별도 comment_extractor.py가 있으면 script directory가
# 우선 검색되고, 없으면 아래 PROJECT_ROOT 경로에서 공통 모듈을 찾는다.
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "누락 Sprinklr Raw Excel을 같은 누락 날짜 폴더 안에서 "
...
def validate_output_directory_name(
    ...
    """
    누락 output 폴더명이 입력 날짜와 일치하는지 검증한다.
    ...
    expected_name = f"{input_date}_누락"

    if output_dir.name != expected_name:
        raise ValueError(
            "지정된 누락 output 폴더명이 입력 날짜와 일치하지 않습니다.\n"
            f"입력 날짜: {input_date}\n"
            f"지정된 output 폴더: {output_dir}\n"
            f"허용 폴더명: {expected_name}"
        )
...
def resolve_output_directory(
    ...
    """
    이번 모듈이 사용할 기존 누락 output 폴더를 확정한다.

    우선순위:
        1. --output-dir
        2. LOCAL_CAMPAIGN_OUTPUT_DIR
        3. 누락/output_누락/{YYMMDD}_누락

    이 함수는 누락 날짜 폴더를 새로 만들지 않는다.
    Sprinklr Export 단계에서 생성된 기존 폴더를 사용한다.
    """
    ...
    if (
        ...
        raise ValueError(
            "누락 run_pipeline.py에서 전달된 작업 날짜와 "
            "입력 날짜가 일치하지 않습니다.\n"
    ...
    if (
        ...
        raise ValueError(
            "--output-dir과 누락 run_pipeline.py가 전달한 "
            "output 경로가 서로 다릅니다.\n"
    ...
    if not output_dir.exists():
        raise FileNotFoundError(
            "누락 실행 output 폴더를 찾을 수 없습니다.\n"
            f"경로: {output_dir}\n"
            "이 모듈은 누락 날짜 폴더를 생성하지 않습니다. "
            "먼저 누락 sprinklr_export_excel.py를 실행해야 합니다."
        )

    if not output_dir.is_dir():
        raise NotADirectoryError(
            "지정된 누락 output 경로가 폴더가 아닙니다.\n"
            f"경로: {output_dir}"
        )
    ...
    if resolved_cli_output_dir is not None:
        print(
            "[INFO] --output-dir로 지정된 기존 누락 "
            "output 폴더를 사용합니다."
        )
    elif environment_output_dir is not None:
        print(
            "[INFO] 누락 run_pipeline.py에서 전달된 "
            "output 폴더를 사용합니다."
        )
    else:
        print(
...
def build_excel_paths(
    ...
    """
    확정된 누락 output 폴더 안에서 입력 및 출력 Excel 경로를 생성한다.
    ...
    출력:
        같은 누락 날짜 폴더 안의 *_formatted.xlsx
    """
